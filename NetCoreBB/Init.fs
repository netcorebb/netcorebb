    ...
    let main args =
        try
            try
                ...
                let builder = builder.ConfigureAppConfiguration(fun hostCtx config ->
                ...
                let builder = builder.UseSerilog().ConfigureLogging(fun hostCtx logBuilder ->
                        ...
                        let msg = [| "+++++ Starting NetCoreBB +++++";
                                  ...
                                  |]
                        for m in msg do Log.Information m |> ignore
                    )


                // [3] Application DI-based services
                let builder = builder.ConfigureServices(fun hostCtx (services: IServiceCollection) ->
                    services.AddControllers() |> ignore
                )


                // [4] Application setup (the rest)
                let builder = builder.Configure(fun webHostBuilder appBuilder ->
